...
.headline-card, .name-tag { position: absolute; box-sizing: border-box; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; overflow: visible; }
...
.caption-layer { position: absolute; z-index: 2; transform: translate(-50%, -50%); display: flex; flex-direction: column; align-items: center; text-align: center; pointer-events: none; }
.caption-line { display: flex; justify-content: center; gap: .32em; white-space: nowrap; }
...
const fitHeadlineFont = (node, style) => {
  ...
  const preferred = Math.max(Number.isFinite(saved) && saved > 0 ? saved : 0, 72);
  const maximum = Math.min(96, Math.max(preferred, 72));
...
const style = data.captionStyle || {};
  ...
  captions.style.maxWidth = clamp(style.maxWidthPercent, 84, 1, 100) + '%';
